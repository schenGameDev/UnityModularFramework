...
using UnityEngine;
using CameraType = ModularFramework.Modules.Camera.CameraType;
...
namespace ModularFramework.Modules.LockTarget
{
    public class LockTargetCamera : MovingCameraBase
    {
        ...
        [FoldoutGroup("Player", nameof(playerOffCenter), nameof(rightSide), nameof(focusRadius),
            nameof(fasterWhenTargetAtScreenEdge), nameof(speedBoostExponential))]
        [SerializeField]
        private Void playerPlaceGroup;

        [FoldoutGroup("Camera Height", nameof(camHeightCurve), nameof(camBackwardCurve), nameof(offCenterCurve),
            nameof(minViewAngleOutOfDistance))]
        [SerializeField]
        ...
        [HideInInspector, SerializeField, Suffix("[0,1]")]
        private AnimationCurve offCenterCurve;

        [HideInInspector, SerializeField, Rename("View Angle Low Limit X Outside Y m"), Suffix("(Horizon 0 deg)")]
        private Vector2 minViewAngleOutOfDistance;


        [FoldoutGroup("Closeup", nameof(closeUpNoRollAngle))] [SerializeField]
        private Void cameraCloseupGroup;
        ...
        [HideInInspector, SerializeField, Range(1, 4), Rename("Roll Acc ^X"),
         ShowField(nameof(fasterWhenTargetAtScreenEdge))]
        ...
        [HideInInspector, SerializeField, Rename("Roll Slow Within"), Suffix("deg")]
        private float closeUpNoRollAngle = 5;

        [HideInInspector, SerializeField, Range(0, 1)]
        private float focusRadius = 0.3f;
        ...
        protected override void Update()
        {
            ...
            if (fasterWhenTargetAtScreenEdge)
            {
                ...
                _rollAccModifier = _targetOffCenterRatio <= 0.5f
                    ? 1
                    : (1 + math.pow((_targetOffCenterRatio - 0.5f) * 10, speedBoostExponential));
            }
        ...
        public bool LostTarget() => !_lockManager.Get().lockOnTarget;
        ...
        private void MoveCamera()
        {
            ...
            var targetToPlayer = _lockManager.Get().lockOnTarget.position - player.position;
            ...
            else
            {
                ...
                if (Vector3.SqrMagnitude(focusPoint.position - leftCamTarget) <= 0.001f ||
                    Vector3.SqrMagnitude(focusPoint.position - rightCamTarget) <= 0.001f)
                {
                    delayTimer = followDelay;
                    FocusPointDecelerate();
                    return;
        ...
        {
            if (!_lockManager.Get().lockOnTarget) return 0;
            Vector2 targetScreenPos = SingletonRegistry<GameBuilder>.Instance.MainCamera.WorldToScreenPoint(_lockManager.Get().lockOnTarget.position);
        ...
        {
            if (!_lockManager.Get().lockOnTarget) return;
            var sqrDist = (_lockManager.Get().lockOnTarget.position - player.position).sqrMagnitude;
            var dir = _lockManager.Get().lockOnTarget.position - focusPoint.position;
        ...
        public override bool Ready => LockTargetExist();
