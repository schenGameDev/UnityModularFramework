...
namespace ModularFramework.Modules.BehaviorTree
{
    public abstract class FindTargetInRangeNode<TTarget> : SwitchNode where TTarget : Component
    {
        ...
        [Tooltip("Number of targets to select"), Min(0)]
        public int number = 1;
        ...
        private bool AnyTargetInRightRange()
        {
            ...
            {
                LogCondition($"{rangeName} no target found.", false);
                return false;
            }

            LogCondition($"{rangeName} target found: " + string.Join(",",targets.Select(t => t.name)), true);
            return true;
        }
        ...
        public override string ToString()
        {
            return base.ToString() +
                   (string.IsNullOrEmpty(rangeName) || base.ToString().Contains(rangeName) ? "" : $" ({rangeName})") +
                   (number <= 1 ? "" : $" [{number}]");
        }
