...
namespace ModularFramework.Modules.BehaviorTree
{
    public abstract class SwitchNode : ControlNode
    {
        ...
        [Tooltip("Verify the condition each frame. If it changes, fail the node")]
        public bool verifyEachFrame;
        ...
        protected override State OnUpdate()
        {
            ...
            {
                if ((afterAction == AfterAction.RUN_YES_AFTER_NO_SUCCESS &&
                     currentRunningChild.parentPortName == PORT_NO) ||
                    (afterAction == AfterAction.RUN_NO_AFTER_YES_SUCCESS &&
                     currentRunningChild.parentPortName == PORT_YES))
                {
        ...
        public override OutputPortDefinition[] OutputPortDefinitions
            => new[]
            {
                new OutputPortDefinition(Port.Capacity.Single, PORT_YES),
