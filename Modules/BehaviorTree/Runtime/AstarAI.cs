...
namespace ModularFramework.Modules.BehaviorTree
{
    ...
    {
        [Header("Config")] private const float NEXT_WAYPOINT_DISTANCE = 1;
        ...
        public uint[] astarTags;


        [Header("Runtime")] [ReadOnly, ShowInInspector]
        private bool _targetFixed;

        [ReadOnly, ShowInInspector, HideField(nameof(_targetFixed))]
        private Transform _target;

        [ReadOnly, ShowInInspector, ShowField(nameof(_targetFixed)), Rename("Target")]
        ...
        private void RecalculatePath()
        {
            ...
            if (Time.time > _lastRepath + REPATH_RATE && seeker.IsDone() &&
                Vector3.SqrMagnitude(tfGroundPos - pos) > 0.01f)
            {
        ...
        private void Update()
        {
            if (SingletonRegistry<GameRunner>.TryGet(out var runner) && runner.IsPause)
            {
                return;
            }
            ...
            var speedFactor = _slowDownAtEnd && reachedEndOfPath
                ? Mathf.Sqrt(Mathf.Sqrt(sqrDistanceToWaypoint) / NEXT_WAYPOINT_DISTANCE)
                : 1f;
