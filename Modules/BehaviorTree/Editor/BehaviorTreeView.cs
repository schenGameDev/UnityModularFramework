...
namespace ModularFramework.Modules.BehaviorTree.Editor
{
    ...
    public partial class BehaviorTreeView : GraphView
    {
        ...
        private GraphViewChange OnGraphViewChanged(GraphViewChange graphViewChange)
        {
            ...
            if (graphViewChange.edgesToCreate != null)
            {
                graphViewChange.edgesToCreate = graphViewChange.edgesToCreate.Where(edge =>
                {
                    ...
                    if (!parentView.Node.IsChildTypeAllowed(childView.Node) ||
                        !childView.Node.IsParentTypeAllowed(parentView.Node))
                    {
                        Debug.LogWarning(
                            $"{childView.Node.GetType()} is not allowed to be child of type {parentView.Node.GetType()}");
                        return false;
                    }
            ...
            {
                HashSet<BTNode> movedNodes =
                    graphViewChange.movedElements.OfType<NodeView>().Select(n => n.Node).ToHashSet();
        ...
        public override void BuildContextualMenu(ContextualMenuPopulateEvent evt)
        {
            ...
            {
                evt.menu.AppendAction("Delete _BACKSPACE", delegate { DeleteSelectionCallback(AskUser.DontAskUser); },
            ...
            if (groupSelectedNodes.Count > 0)
            {
                ...
                foreach (var nodeView in groupSelectedNodes)
                {
                    ...
                    BTNode node = _tree.DuplicateNode(nodeView.Node,
                        nodeView.GetPosition().position + new Vector2(10, 10));
