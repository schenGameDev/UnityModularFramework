...
namespace ModularFramework.Modules.Sound
{
    ...
    {
        [Header("Config")] public SoundProfileBucket tracks;

        [Header("Music")] [SerializeField] private float crossFadeTime = 1.0f;
        [SerializeField] public bool repeat = true;

        [SerializeField, ShowField(nameof(repeat)), Suffix("repeat the first track")]
        public bool repeatOnAwake = true;

        [SerializeField] List<string> defaultPlaylist = new();

        [FoldoutGroup("Event Channels", nameof(trackChannel))] [SerializeField]
        private Void eventChannelGroup;

        [HideInInspector, SerializeField] StringEventChannelSO trackChannel;

        [Header("Runtime")] [ReadOnly, SerializeField, RuntimeObject, Rename("Current Track")]
        ...
        [SerializeField, ReadOnly, RuntimeObject]
        ...
        async UniTaskVoid CrossFade(float duration, CancellationToken token)
        {
            ...
            while (fraction < 1)
            {
                ...
                if (PrevTrack && MusicPlayers[1])
                    MusicPlayers[1].volume = Mathf.Min(MusicPlayers[1].volume, 1 - logFraction);
                if (CurrentTrack && MusicPlayers[0])
                    MusicPlayers[0].volume = Mathf.Max(MusicPlayers[0].volume, logFraction);
        ...
        {
            await UniTask.WaitWhile(() => audio && audio.isPlaying, cancellationToken: token)
