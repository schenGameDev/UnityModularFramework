...
namespace ModularFramework.Modules.Quest
{
    ...
    {
        [Header("Config")] [SerializeField] private SOBucket<Quest> quests;

        [SerializeField] private EventChannel<(string, QuestStage)> questChannel;
        ...
        public void SaveQuestProgress()
        {
            ...
            quests.ForEach(q =>
            {
                if (q.stage != QuestStage.UNKNOWN) questProgress.Add(q.name, (int)q.stage);
            });

            if (questProgress.NonEmpty())
                SaveUtil.SaveState(QuestConstants.KEY_QUEST, JsonUtility.ToJson(questProgress));

            _milestones.Values.Where(m => m.Reached.HasValue)
                .ForEach(m => milestoneProgress.Add(m.id, m.Reached.Value));

            if (milestoneProgress.NonEmpty())
                SaveUtil.SaveState(QuestConstants.KEY_QUEST_MILESTONE, JsonUtility.ToJson(milestoneProgress));
        }
        ...
        private void UpdateQuestStage((string, QuestStage) request) =>
            UpdateQuestStage(request.Item1, request.Item2);

        public void UpdateQuestStage(string questName, QuestStage stage)
        {
            quests.Get(questName).Do(q =>
            {
                if (q.stage == QuestStage.COMPLETED || q.stage == QuestStage.FAILED)
                    return; // can't alter completed or failed task
                q.stage = stage;
                string activity;
                if (stage == QuestStage.FAILED)
                {
        ...
        public List<Quest> GetActiveQuests()
        {
            ...
            quests.ForEach(q =>
            {
                if (q.stage == QuestStage.ACTIVE) activeQuests.Add(q);
            });
            return activeQuests;
        }
        ...
        public List<Quest> GetKnownQuests()
        {
            ...
            quests.ForEach(q =>
            {
                if (q.stage == QuestStage.ACTIVE ||
                    q.stage == QuestStage.COMPLETED ||
                    q.stage == QuestStage.FAILED) knownQuests.Add(q);
            });
            return knownQuests;
        }
    }
