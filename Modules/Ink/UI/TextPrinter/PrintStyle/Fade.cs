...
namespace ModularFramework.Modules.Ink
{
    ...
    {
        [Header("Config")] [SerializeField] private float fadeInDuration = 0.5f;
        ...
        private async UniTaskVoid FadeIn(Action callback, CancellationToken token)
        {
            ...
            if (displayDuration > 0)
                await UniTask.WaitForSeconds(displayDuration, cancellationToken: token).SuppressCancellationThrow();
