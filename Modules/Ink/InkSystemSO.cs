...
using ModularFramework.Modules.Quest;
...
namespace ModularFramework.Modules.Ink
{
    ...
    public class InkSystemSO : GameSystem<InkSystemSO>
    {
        ...
        [Header("Config")] [SerializeField] private InkStoryBucket stories;
        ...
        [Header("Runtime")] [Rename("Current Story"), ReadOnly, SerializeField, RuntimeObject]
        string currentStoryName;

        [ReadOnly, SerializeField, RuntimeObject]
        ...
        [RuntimeObject(cleaner: nameof(ClearStory))]
        ...
        public InkStage Next(int choiceIndex = -1)
        {
            // next button clickable only at READY state
            if (TaskRunning())
            {
            ...
            if (stage == InkStage.READY)
            {
                ...
                else
                {
                    ...
                    List<InkTag> tags = _currentStory.currentTags == null
                        ? new()
                        : _currentStory.currentTags.Select(t => InkTag.Of(t, tagDefBuckets, Get)).ToList();
        ...
        private InkChoice NextChoice()
        {
            ...
            for (int i = 0; i < _currentStory.currentChoices.Count; i++)
            {
                ...
                List<InkTag> tags = choice.tags == null
                    ? new()
                    : choice.tags.Select(t => InkTag.Of(t, tagDefBuckets, Get)).ToList();
        ...
        {
            SaveUtil.GetState(InkConstants.KEY_AUTO_PLAY_SPEED)
                .Do(s => InkUIIntegrationSO.PlaySpeed = s.GetEnumValue<InkUIIntegrationSO.AutoPlaySpeed>());
        ...
        private void RaiseTask(string taskHandler, string parameter)
        {
                ...
                taskHandler == InkConstants.TASK_COMPLETE_QUEST)
            {
                ...
                if (sys)
                {
                    QuestStage s = taskHandler switch
                    {
                        InkConstants.TASK_DROP_QUEST => QuestStage.FAILED,
                        InkConstants.TASK_COMPLETE_QUEST => QuestStage.COMPLETED,
                        _ => QuestStage.ACTIVE
                    };
        ...
        public bool CanSave => !TaskRunning();
        [RuntimeObject] private List<(string, string, bool)> _taskHistory = new(); // clear when scence load
                ...
                .Do(states =>
                {
                    ...
                    FindObjectsByType<MonoBehaviour>(FindObjectsInactive.Include, FindObjectsSortMode.None)
                        ...
                        .ForEach(savable => history.Get(savable.Id)
                            .Do(dict =>
                            {
                                savable.RestoreState(dict);
                                savable.Load();
                            }).OrElseDo(() => Debug.LogWarning($"{savable.Id} is not saved")));
                });
        }

        [Button]
        private void ValidateSavable()
        {
            ...
            {
                warnings.ForEach(Debug.LogWarning);
                warnings.Clear();
            }
