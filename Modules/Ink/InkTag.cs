...
namespace ModularFramework.Modules.Ink
{
    ...
    public class InkTag
    {
        ...
        public static InkTag Of(string tagText, InkTagDefBucket[] tagDefBuckets,
            Func<string, Optional<Keeper>> getKeeperFunc)
        {
            if (tagText.StartsWith(CONDITION_TAG_PREFIX))
            {
                ...
                do
                {
                    ...
                    if (right != -1)
                    {
                        ...
                        {
                            expression = expression.Replace(unitExpression,
                                SimpleBoolEvaluator(unitExpression, getKeeperFunc));
                        }
                        else if (ESCAPE_COMPARE_CHARS.Any(c => unitExpression.Contains(c)))
                        {
                            expression = expression.Replace(unitExpression,
                                SimpleCompareEvaluator(unitExpression, getKeeperFunc));
                        }
                        else if (ESCAPE_MATH_CHARS.Any(c => unitExpression.Contains(c)))
                        {
                            expression = expression.Replace(unitExpression,
                                SimpleMathEvaluator(unitExpression, getKeeperFunc));
                        }
                        else
                        {
                    ...
                    expression = ESCAPE_LOGIC_CHARS.Any(c => expression.Contains(c))
                        ? SimpleBoolEvaluator(expression, getKeeperFunc)
                        : SimpleCompareEvaluator(expression, getKeeperFunc);
                } while (right != -1);
        ...
        private static string SimpleMathEvaluator(string expression, Func<string, Optional<Keeper>> getKeeperFunc)
        {
            ...
            List<string> operands =
                expression.Where(ch => ESCAPE_MATH_CHARS.Contains(ch)).Select(ch => "" + ch).ToList();
