...
namespace ModularFramework.Modules.Camera
{
    ...
    {
        [SerializeField, OnValueChanged(nameof(UpdateFollowOffset))]
        protected Vector3 cameraOffSet = new(0, 0, 0);

        [FoldoutGroup("Camera Follow", nameof(followAcceleration), nameof(friction), nameof(followMaxSpeed),
            nameof(followDelay))]
        [SerializeField]
        private Void cameraFollowGroup;

        [HideInInspector, SerializeField, Rename("Friction"), Suffix("m/s2")]
        protected float friction = 0.3f;

        [HideInInspector, SerializeField, Rename("Follow Max Speed"), Suffix("m/s")]
        protected float followMaxSpeed = 10;

        [HideInInspector, SerializeField, Rename("Acceleration"), Suffix("m/s2")]
        protected float followAcceleration = 2;

        [HideInInspector, SerializeField, Rename("Delay"), Suffix("s")]
        protected float followDelay = 0;

        [FoldoutGroup("Camera Roll", nameof(rollStartSpeed), nameof(rollMaxSpeed), nameof(rollAcceleration))]
        [SerializeField]
        private Void cameraRollGroup;

        [HideInInspector, Rename("Start Speed"), SerializeField, Suffix("deg/s")]
        protected float rollStartSpeed = 10;

        [HideInInspector, Rename("Max Speed"), SerializeField, Suffix("deg/s")]
        protected float rollMaxSpeed = 60;

        [HideInInspector, Rename("Acceleration"), SerializeField, Suffix("deg/s2")]
        protected float rollAcceleration = 20;
        ...
        protected override void MatchPrevCamPosition()
        {
            ...
            if (cameraManager.Get().transitionAcceleration
                .TryGetValue(new Vector<string>(prevName, curName), out float accModifier))
            {
        ...
        protected void RollAccelerate(float accelerationModifier = 1f)
        {
            ...
            else if (rollSpeed < rollMaxSpeed)
                rollSpeed = Mathf.Min(rollSpeed + accelerationModifier * rollAcceleration * Time.deltaTime,
                    rollMaxSpeed);
